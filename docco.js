...
(function() {
  var Docco, async, commander, config, configure, document, ext, format, fs, getLanguage, highlight, l, languages, marked, parse, path, run, version, write, _,
    __slice = [].slice;
    ...
    return fs.mkdirs(config.output, function() {
      ...
      complete = function() {
        return async.parallel([
          function(done) {
            return fs.copy(config.css, config.output, done);
          }, function(done) {
            if (fs.existsSync(config["public"])) {
              return fs.copy(config["public"], config.output, done);
            } else {
              return done();
            }
          }
