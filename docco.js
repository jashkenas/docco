...
(function() {
  var Docco, _, buildMatchers, commander, configure, defaults, document, format, fs, getDestinations, getLanguage, glob, highlightjs, languages, marked, parse, path, run, version, write;
  ...
  getDestinations = function(config) {
    var asource, asourcetToDirectory, cssPath, cssRelative, destinations, from, j, k, lang, len, len1, linkPath, ref, ref1, relativeLink, source, sourceNoExt, toDirectory, toExtName, toFile, toLinkBasenameNoExt, toLinkExtName, toSources;
    destinations = [];
    ref = config.sources;
    for (j = 0, len = ref.length; j < len; j++) {
      source = ref[j];
