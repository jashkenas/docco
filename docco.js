...
(function() {
  ...
  var Docco, MarkdownIt, _, buildMatchers, commander, configure, defaults, document, format, fs, getLanguage, hljs, languages, md, parse, path, run, version, write;
  ...
  format = function(source, sections, config) {
    var code, i, j, language, len, results, section;
...
// if not specified.
    ...
    for (i = j = 0, len = sections.length; j < len; i = ++j) {
      ...
      results.push(section.docsHtml = md.render(section.docsText));
    }
    return results;
  };
  ...
  write = function(source, sections, config) {
    var css, destination, html, relative, title;
