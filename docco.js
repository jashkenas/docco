...
(function() {
  ...
  document = function(options, callback) {
    ...
    return exec("mkdir -p " + config.output, function() {
      ...
      complete = function() {
        return exec(["cp -f " + config.css + " " + config.output, "cp -f " + config.js + " " + config.output, fs.existsSync(config["public"]) ? "cp -fR " + config["public"] + " " + config.output : void 0].join(' && '), callback);
      };
  ...
  configure = function(options) {
    ...
    if (options.template) {
      ...
      config.js = options.js || ("" + dir + "/docco.js");
    }
