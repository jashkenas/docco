...
(function() {
  var commander, defaults, document, ensureDirectory, exec, ext, fs, generateDocumentation, generateHtml, getLanguage, getResource, highlight, highlightEnd, highlightStart, htmlEscape, l, languages, marked, parse, path, resolveSource, run, spawn, version, _, _ref,
  ...
  parse = function(source, code, config) {
    var codeText, docsText, hasCode, i, language, line, lines, match, save, sections, _i, _j, _len, _len1;
    lines = code.split('\n');
    sections = [];
    ...
    if (language.literate) {
      for (i = _i = 0, _len = lines.length; _i < _len; i = ++_i) {
        line = lines[i];
        lines[i] = (match = /^([ ]{4}|\t)/.exec(line)) ? line.slice(match[0].length) : '# ' + line;
      }
    }
