(function() {
  ...
  parse = function(source, code, config) {
    var code_text, docs_text, has_code, in_block, language, line, lines, save, sections, _i, _len;
    lines = code.split('\n');
    sections = [];
    ...
    for (_i = 0, _len = lines.length; _i < _len; _i++) {
      ...
      if (!line.match(language.comment_filter) && (in_block || line.match(language.comment_matcher))) {
        if (has_code) {
          save(docs_text, code_text);
          has_code = docs_text = code_text = '';
        }
        if (config.blocks && language.blocks && line.match(language.exit)) {
          in_block = false;
