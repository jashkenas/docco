(function() {
  var argv, dest_dir, destination, destination_path, docco_styles, docco_template, ensure_directory, exec, ext, fs, generate_documentation, generate_html, get_language, highlight, highlight_end, highlight_start, l, languages, optimist, parse, path, showdown, sources, spawn, template, _ref;
  ...
  optimist = require('optimist').usage("docco -- the quick'n'dirty literate-programming style documenting system\n\nUsage: docco [options] file file ...").string('o')["default"]('o', 'docs').alias('o', 'output-dir').describe('o', 'specifies the output directory').boolean('h').alias('h', 'help').describe('h', 'show this message');
  ...
  destination_path = function(filename) {
    return path.join(dest_dir, path.basename(filename));
  };
  ...
  if (sources.length) {
    ensure_directory(dest_dir, function() {
      var files, next_file;
