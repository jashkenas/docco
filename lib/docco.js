(function() {
  ...
  parse = function(source, code) {
    var code_text, docs_text, hasSeenOpeningDocComment, has_code, language, line, lines, save, sections, tline, _i, _len;
    lines = code.split('\n');
    sections = [];
    ...
    for (_i = 0, _len = lines.length; _i < _len; _i++) {
      line = lines[_i];
      if (line === '###' || hasSeenOpeningDocComment) {
        if (!hasSeenOpeningDocComment) {
          hasSeenOpeningDocComment = true;
          continue;
        } else if (hasSeenOpeningDocComment && line === '###') {
          hasSeenOpeningDocComment = false;
          continue;
        } else {
