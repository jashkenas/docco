(function() {
  var VERSION, destination, docco_styles, docco_template, ensure_directory, exec, ext, fs, generate_documentation, generate_html, get_language, highlight, highlight_end, highlight_start, l, languages, options, parent, parse, parser, path, showdown, sources, spawn, template, _ref, _ref2;
  VERSION = '0.3.0';
  ...
  destination = function(filepath) {
    return path.join(parent, path.basename(filepath, path.extname(filepath)) + '.html');
  };
  ensure_directory = function(dir, callback) {
    return exec("mkdir -p " + (dir.replace(' ', '\\ ')), function() {
      return callback();
    });
  };
  ...
  path = require('path');
  parser = new (require('./optparse')).OptionParser([['-o [output]', '--output [output]', 'set the directory for output documentation'], ['-S [styles]', '--styles [styles]', 'set the path to the stylesheet used for documentation'], ['-v', '--version', 'display docco version'], ['-h', '--help', 'display this help message']], "Usage: docco [options] path/to/scripts");
  options = parser.parse(process.argv);
  if (options.help) {
    console.log(parser.help());
  }
  if (options.version || !options.arguments) {
    console.log("Docco version " + VERSION);
  }
  docco_styles = fs.readFileSync(options.styles ? path.resolve(process.cwd(), options.styles) : __dirname + '/../resources/docco.css').toString();
  parent = path.resolve(process.cwd(), (_ref2 = options.output) != null ? _ref2 : "docs");
  sources = options.arguments.sort();
  if (sources.length) {
    ensure_directory(parent, function() {
      var files, next_file;
