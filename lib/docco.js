...
(function() {
  var commander, defaults, document, ensureDirectory, exec, ext, fs, generateDocumentation, generateHtml, getLanguage, getResource, hasProgram, highlightEnd, highlightJsHighlight, highlightStart, key, l, languages, parse, path, pygmentsHighlight, resolveSource, run, showdown, spawn, template, value, version, _ref, _ref1,
  ...
  generateHtml = function(source, sections, config) {
    ...
    destination = function(filepath) {
      return path.join(config.output, path.dirname(filepath).replace(/\//, '_') + '_' + path.basename(filepath, path.extname(filepath)) + '.html');
    };
